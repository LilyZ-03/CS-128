...
pub fn get_attendance(students: Vec<&Student>, class: &Class) -> Option<Vec<(Student, bool)>> {
    let mut enrolled: Vec<(Student, bool)> = Vec::new();
    let mut empty: bool = true;
    for s in students.iter() {
    let mut enrl: bool = false;
    for e in class.attendance.iter() {
        if e.netid == s.netid {
            enrl = true;
            empty = false;
            break;
        ...
        let r: Student = student_init(s.name.clone(), s.netid.clone());
        enrolled.push((r, enrl));
    }
    match empty {
        true => return None,
        false => return Some(enrolled)
    }
}
