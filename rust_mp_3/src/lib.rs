    ...
    pub fn word_count_mapper(words: &Vec<String>) -> HashMap<String, usize> {
        ...
        for word in words.iter() {
            if map.contains_key(word) {
                map.insert(String::from(word), map[word] + 1);
            } else {
...
pub mod map {
    ...
    pub fn split_into_chunks(items: &Vec<String>, num_chunks: usize) 
        ...
        let basic_len: usize = items.len() / num_chunks;
        let num_of_long_chunks: usize = items.len() % num_chunks;
        let mut index: usize = 0;
        for i in 0..num_chunks {
            let mut vec: Vec<String> = Vec::new();
            let mut iter_range: usize = basic_len;
            if i < num_of_long_chunks {
                iter_range += 1;
            }
            for j in 0..iter_range {
                vec.push(String::from(items[index].clone()));
                index += 1;
            }
            res_vec.push(vec);
        }
    ...
    pub fn multi_threaded_mapper(words: &Vec<String>, num_chunks: usize) 
        ...
        let (tx, rx) = std::sync::mpsc::channel::<HashMap<usize, usize>>();
        let splitted_vec: Vec<Vec<String>> = split_into_chunks(words, num_chunks);
        for chunk in splitted_vec {
            let tx_thread = tx.clone();
            let handle = thread::spawn(move || {
                let res_len: HashMap<usize, usize> = word_length_mapper(&chunk);
                tx_thread.send(res_len).unwrap()
            });
            handles.push(handle);
        }
        (handles, rx)
    ...
    pub fn multi_threaded_mapper_generic<KeyType: 'static + Eq + Hash + Send>(
        ...
        let mut handles: Vec<JoinHandle<()>> = Vec::new();            
        let (tx, rx) = std::sync::mpsc::channel::<HashMap<KeyType, usize>>();
        let splitted_vec: Vec<Vec<String>> = split_into_chunks(words, num_chunks);
        for chunk in splitted_vec {
            let tx_thread = tx.clone();
            let handle = thread::spawn(move || {
                let res: HashMap<KeyType, usize> = func(&chunk);
...
pub mod reduce {
    ...
    pub fn thread_reducer<KeyType: Eq + Hash>(
        ...
        for recv in receivers.0 {
            for (key, value) in receivers.1.recv().unwrap().into_iter() {
                if res.contains_key(&key) {
                    *(res.get_mut(&key).unwrap()) += value;
                } else {
