...
pub fn reverse_word(word: &mut String) {
    ...
    for i in 0..char_vec.len() {
        reversed.push(*char_vec.get(char_vec.len() - 1 - i).unwrap());
    }
    *word = reversed.clone();
}
...
pub fn count_vowels(word: &str) -> i32 {
    let mut total: i32 = 0;
    let word_vec: Vec<char> = String::from(word).to_lowercase().chars().collect();
...
mod test {
    ...
    fn test_count_vowels() {
        let word = "A!weeadlpiouspsndjfogha";
        assert_eq!(count_vowels(word), 9);
    }
